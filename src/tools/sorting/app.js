    ...
    class SortingVisualizer {
        ...
        bind() {
            ...
            controls.defaults.addEventListener("click", () => {
                controls.algorithm.value = "bubble";
                controls.size.value = 24;
                controls.speed.value = 5;
                this.updateAlgorithm();
                this.randomize();
            });
        ...
        setData(values, message) {
            ...
            controls.size.value = values.length;
            controls.values.value = values.join(", ");
            this.clearStats();
            this.draw();
        ...
        preset(kind) {
            ...
            let values;
            if (kind === "reverse") values = Array.from({
                length: n
            }, (_, i) => Math.round(5 + (94 * (n - i - 1)) / Math.max(1, n - 1)));
            else if (kind === "few") values = Array.from({
                length: n
            }, () => [20, 40, 60, 80][Math.floor(Math.random() * 4)]);
            else {
                values = Array.from({
                    length: n
        ...
        stop() {
            ...
            this.paused = false;
            this.stepCredits = 0;
            controls.pause.innerHTML = '<span class="btn-icon">⏸️</span> Pause';
        }
        clearStats() {
            ...
            this.startedAt = 0;
            this.stats();
            this.operation("Ready");
        }
        updateAlgorithm() {
            $("algorithm-name").textContent = names[controls.algorithm.value];
        }
        operation(text) {
            controls.operation.textContent = text;
        }
        stats() {
            $("swaps-count").textContent = this.swaps;
        ...
        draw(active = [], special = [], message) {
            ...
            const w = this.width,
                h = this.height,
                n = this.array.length,
                pad = Math.max(18, w * .045),
                top = Math.max(42, h * .1),
                base = h - pad;
            ctx.clearRect(0, 0, w, h);
            const gradient = ctx.createLinearGradient(0, 0, 0, h);
            gradient.addColorStop(0, "rgba(56,198,194,.10)");
            gradient.addColorStop(1, "rgba(56,198,194,.015)");
            ctx.fillStyle = gradient;
            ctx.fillRect(0, 0, w, h);
            ctx.strokeStyle = color("grid", "rgba(148,163,184,.16)");
            ctx.lineWidth = 1;
            for (let i = 1; i < 5; i++) {
            ...
            this.array.forEach((value, i) => {
                const barH = Math.max(3, ((base - top) * value) / max),
                    x = pad + i * slot + gap / 2,
                    y = base - barH;
                ...
                ctx.shadowBlur = active.includes(i) || special.includes(i) ? 14 : 3;
                ctx.fillStyle = fill;
                ctx.beginPath();
                const radius = Math.min(5, barW / 2);
                ctx.roundRect(x, y, barW, barH, [radius, radius, 1, 1]);
                ctx.fill();
                ctx.shadowBlur = 0;
                if (slot >= 24 || n <= 16) {
                    ctx.fillStyle = y > top + 22 ? "#fff" : color("text", "#dbeafe");
        ...
        async checkpoint(active = [], special = [], message = "Comparing") {
            ...
            this.draw(active, special, message);
            this.stats();
            ...
            await new Promise((r) => setTimeout(r, 230 - speed * 22));
            return this.running;
        }
        ...
        togglePause() {
            ...
            controls.pause.innerHTML = this.paused ? '<span class="btn-icon">▶️</span> Resume' : '<span class="btn-icon">⏸️</span> Pause';
        }
        step() {
            if (!this.running) {
        ...
        async start(stepStart = false) {
            ...
            this.running = true;
            this.paused = false;
            this.manualStep = stepStart;
            this.stepCredits = stepStart ? 1 : 0;
            this.completed.clear();
            this.swaps = 0;
            this.comparisons = 0;
            ...
            this.operation(`${names[algorithm]} sort running`);
            await this[`${algorithm}Sort`]();
            if (!this.running || id !== this.runId) return;
            this.running = false;
            this.manualStep = false;
            this.completed = new Set(this.array.map((_, i) => i));
            this.draw([], [], "Sorted ✓");
            this.stats();
            controls.values.value = this.array.join(", ");
        ...
        async bubbleSort() {
            ...
            for (let end = n - 1; end > 0; end--) {
                ...
                for (let j = 0; j < end; j++) {
                    this.comparisons++;
                    if (!await this.checkpoint([j, j + 1], [], `Compare ${this.array[j]} and ${this.array[j + 1]}`)) return;
        ...
        async selectionSort() {
            for (let i = 0; i < this.array.length; i++) {
                ...
                for (let j = i + 1; j < this.array.length; j++) {
                    this.comparisons++;
                    if (!await this.checkpoint([j], [min], `Scan for the minimum after index ${i}`)) return;
        ...
        async insertionSort() {
            this.completed.add(0);
            for (let i = 1; i < this.array.length; i++) {
                const key = this.array[i];
                let j = i - 1;
                while (j >= 0) {
                    this.comparisons++;
                    if (!await this.checkpoint([j], [j + 1], `Insert ${key} into the sorted prefix`)) return;
        ...
        async mergeSort(lo = 0, hi = this.array.length - 1) {
            if (lo >= hi || !this.running) return;
            const mid = (lo + hi) >> 1;
            await this.mergeSort(lo, mid);
            await this.mergeSort(mid + 1, hi);
            const left = this.array.slice(lo, mid + 1),
                right = this.array.slice(mid + 1, hi + 1);
            let a = 0,
                b = 0;
            for (let k = lo; k <= hi; k++) {
                if (a < left.length && b < right.length) this.comparisons++;
                if (b >= right.length || a < left.length && left[a] <= right[b]) this.array[k] = left[a++];
                else this.array[k] = right[b++];
                this.swaps++;
                if (!await this.checkpoint([k], Array.from({
                        length: hi - lo + 1
                    }, (_, x) => lo + x), `Merge positions ${lo + 1}–${hi + 1}`)) return;
            }
        }
        async quickSort(lo = 0, hi = this.array.length - 1) {
            if (lo >= hi || !this.running) {
                if (lo === hi) this.completed.add(lo);
                return;
            }
            ...
            for (let j = lo; j < hi; j++) {
                this.comparisons++;
                if (!await this.checkpoint([j], [hi], `Compare ${this.array[j]} with pivot ${pivot}`)) return;
            ...
            } [this.array[i], this.array[hi]] = [this.array[hi], this.array[i]];
            this.swaps++;
            this.completed.add(i);
            await this.quickSort(lo, i - 1);
            await this.quickSort(i + 1, hi);
        }
        async heapSort() {
            const n = this.array.length;
            const heapify = async (size, root) => {
                while (this.running) {
                    ...
                    if (!await this.checkpoint([root, largest], [], "Restore the max heap")) return;
                    if (largest === root) return;
                    [this.array[root], this.array[largest]] = [this.array[largest], this.array[root]];
                    this.swaps++;
                    root = largest;
                }
            };
            for (let i = (n >> 1) - 1; i >= 0; i--) await heapify(n, i);
            for (let end = n - 1; end > 0 && this.running; end--) {
        ...
        async radixSort() {
            ...
            for (let exp = 1; Math.floor(max / exp) > 0 && this.running; exp *= 10) {
                const buckets = Array.from({
                    length: 10
                }, () => []);
                for (let i = 0; i < this.array.length; i++) buckets[Math.floor(this.array[i] / exp) % 10].push(this.array[i]);
                const output = buckets.flat();
                for (let i = 0; i < output.length; i++) {
                    this.array[i] = output[i];
                    this.swaps++;
                    if (!await this.checkpoint([i], [], `Place by ${exp === 1 ? "ones" : exp === 10 ? "tens" : "hundreds"} digit`)) return;
                }
            }
        }
    }
