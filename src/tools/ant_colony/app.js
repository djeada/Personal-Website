(() => {
    ...
    let home = new Float32Array(cols * rows),
        nectar = new Float32Array(cols * rows);
    ...
    class Ant {
        ...
        reset(fresh = false) {
            const a = Math.random() * TAU,
                r = Math.random() * nest.radius * .75;
            this.x = nest.x + Math.cos(a) * r;
            this.y = nest.y + Math.sin(a) * r;
            this.angle = fresh ? a : Math.random() * TAU;
            this.target = this.angle;
            this.carrying = false;
            this.speed = .85 + Math.random() * .5;
            this.seed = Math.random() * 100;
            this.stuckFrames = 0;
            this.steps = Math.floor(Math.random() * 4);
        }
        sense(field, offset) {
        ...
        blocked(x, y, padding = 2) {
            return obstacles.some(o => Math.hypot(x - o.x, y - o.y) < o.r + padding);
        }
        avoidObstacles() {
            const look = 18;
            const ahead = this.blocked(this.x + Math.cos(this.angle) * look, this.y + Math.sin(this.angle) * look, 4);
            if (!ahead) return;
            const leftAngle = this.angle - .9,
                rightAngle = this.angle + .9;
        ...
        update() {
            ...
            const left = this.sense(field, -.62),
                center = this.sense(field, 0),
                right = this.sense(field, .62);
            ...
            const step = this.speed * 1.35,
                nx = this.x + Math.cos(this.angle) * step,
                ny = this.y + Math.sin(this.angle) * step;
            if (nx < 8 || nx > W - 8 || ny < 8 || ny > H - 8 || this.blocked(nx, ny)) {
                this.stuckFrames++;
                this.target += (this.seed % 2 > 1 ? 1 : -1) * (1.05 + Math.random() * .55);
                ...
                if (this.stuckFrames > 20) {
                    ...
                    this.target = escape + (Math.random() - .5) * .7;
                    this.angle = this.target;
                    this.stuckFrames = 0;
                }
            } else {
            ...
            if (!this.carrying)
                for (const source of food) {
                    ...
                    if (source.amount > 0 && distance(this, source) < harvestRadius) {
                        ...
                        this.angle += Math.PI;
                        this.target = this.angle;
                        nectar[gridIndex(source.x, source.y)] = 255;
                        break;
                    }
                }
        ...
        draw() {
            ...
            [
                ...
                ctx.beginPath();
                ctx.moveTo(px, side * 1.5);
                ctx.lineTo(px - 1 + leg * (i % 2 ? -1 : 1), side * 4);
                ctx.stroke();
            });
            ctx.shadowBlur = this.carrying ? 9 : 2;
            ctx.shadowColor = this.carrying ? "#ffd36e" : "#f4b46d";
            ctx.fillStyle = this.carrying ? "#ffd268" : "#d9894f";
            ctx.strokeStyle = "#281914";
            ctx.lineWidth = .8;
            ctx.beginPath();
            ctx.ellipse(-2.5, 0, 2.6, 1.9, 0, 0, TAU);
            ctx.ellipse(1, 0, 1.9, 1.55, 0, 0, TAU);
            ctx.ellipse(3.8, 0, 1.55, 1.35, 0, 0, TAU);
            ctx.fill();
            ctx.stroke();
            if (this.carrying) {
                ctx.fillStyle = "#dfff92";
                ctx.beginPath();
                ctx.arc(6.2, 0, 1.8, 0, TAU);
                ctx.fill();
            }
            ctx.restore();
            if (settings.sensors) {
                ...
                [-.62, 0, .62].forEach(o => {
                    ...
                    ctx.lineTo(this.x + Math.cos(this.angle + o) * 28, this.y + Math.sin(this.angle + o) * 28);
                    ctx.stroke();
                });
            }
        }
    ...
    function updateTrails() {
        ...
        for (let i = 0; i < home.length; i++) {
            home[i] = home[i] < .3 ? 0 : home[i] * fade;
            nectar[i] = nectar[i] < .3 ? 0 : nectar[i] * fade;
        }
    }

    function drawGround() {
        const bg = ctx.createRadialGradient(410, 300, 20, 400, 300, 520);
        bg.addColorStop(0, "#22352f");
        bg.addColorStop(.48, "#142421");
        bg.addColorStop(1, "#0a1115");
        ctx.fillStyle = bg;
        ctx.fillRect(0, 0, W, H);
        ctx.save();
        ctx.globalAlpha = .18;
        ctx.strokeStyle = "#61715c";
        ctx.lineWidth = 1;
        for (let i = 0; i < 32; i++) {
            const y = (i * 83) % H,
                x = (i * 137) % W;
            ctx.beginPath();
            ctx.moveTo(x - 65, y);
            ctx.bezierCurveTo(x - 20, y - 22, x + 22, y + 27, x + 85, y - 6);
            ctx.stroke();
        }
        ctx.restore();
        ...
        for (let r = 0; r < rows; r++)
            for (let c = 0; c < cols; c++) {
                const i = r * cols + c,
                    ...
                    b = home[i];
                if (a > 8) {
                    ctx.fillStyle = `rgba(177,255,91,${Math.min(.24, a / 850)})`;
                    ctx.beginPath();
                    ctx.arc(c * GRID + 4, r * GRID + 4, 3.1, 0, TAU);
                    ctx.fill();
                }
                if (b > 12) {
                    ctx.fillStyle = `rgba(139,122,255,${Math.min(.13, b / 1350)})`;
                    ctx.beginPath();
                    ctx.arc(c * GRID + 4, r * GRID + 4, 2.6, 0, TAU);
                    ctx.fill();
                }
            }
        ctx.restore();
    ...
    function drawMotes() {
        ...
        motes.forEach(m => {
            m.y -= m.drift;
            m.x += Math.sin(frame * m.speed + m.phase) * .12;
            if (m.y < -5) {
                m.y = H + 5;
                m.x = Math.random() * W;
            }
            ...
            ctx.shadowBlur = 8;
            ctx.shadowColor = m.gold ? "#ffc966" : "#91eecb";
    ...
    function drawNest() {
        ...
        ctx.shadowBlur = 30;
        ctx.shadowColor = "rgba(151,112,255,.55)";
        roundedPolygon(0, 0, Array.from({
            length: 12
        }, (_, i) => [i / 12 * TAU, i % 2 ? 43 : 48]), "#17201d", "#66558b");
        ctx.shadowBlur = 0;
        ctx.lineWidth = 2;
        ...
        hole.addColorStop(0, "#000305");
        hole.addColorStop(.72, "#080b0d");
        hole.addColorStop(1, "#392d4b");
        ctx.fillStyle = hole;
        ctx.beginPath();
        ctx.arc(0, 0, 29, 0, TAU);
        ctx.fill();
        ctx.fillStyle = "rgba(204,184,255,.75)";
        ctx.font = "600 9px Georgia";
        ctx.textAlign = "center";
        ctx.fillText("GLOAMROOT", 0, 4);
        ctx.restore();
    }
    ...
    function drawFood() {
        food.forEach((f, fi) => {
            ...
            const scale = .12 + .88 * Math.sqrt(remaining),
                pulse = 1 + Math.sin(frame * .025 + fi * 2) * .05;
            ...
            ctx.globalCompositeOperation = "lighter";
            ctx.shadowBlur = 24;
            ctx.shadowColor = `hsla(${f.hue},90%,65%,.75)`;
            for (let i = 0; i < shardCount; i++) {
                const a = i / shardCount * TAU + fi,
                    h = 12 + (i % 3) * 6;
                ctx.fillStyle = `hsla(${f.hue + i * 4},75%,${52 + i * 3}%,.72)`;
                ctx.beginPath();
                ctx.moveTo(Math.cos(a) * 5, Math.sin(a) * 5);
                ctx.lineTo(Math.cos(a - .2) * h, Math.sin(a - .2) * h);
                ctx.lineTo(Math.cos(a + .2) * h, Math.sin(a + .2) * h);
                ctx.closePath();
                ctx.fill();
            }
            ctx.fillStyle = "rgba(235,255,203,.85)";
            ctx.beginPath();
            ctx.arc(0, 0, 6, 0, TAU);
            ctx.fill();
            ctx.restore();
    ...
    function drawObstacles() {
        obstacles.forEach((o, i) => {
            ...
            ctx.rotate(o.angle);
            ctx.shadowBlur = 9;
            ctx.shadowColor = "rgba(99,212,180,.25)";
            roundedPolygon(0, 0, o.points, "#202a28", "#55746b");
            ctx.strokeStyle = "rgba(125,214,181,.45)";
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(-o.r * .35, o.r * .25);
            ctx.lineTo(0, -o.r * .35);
            ctx.lineTo(o.r * .28, o.r * .2);
            ctx.stroke();
            ctx.restore();
        });
    }
    ...
    function loop() {
        frame++;
        for (let s = 0; s < Math.max(1, Math.round(settings.speed)); s++) ants.forEach(a => a.update());
        updateTrails();
        drawGround();
        drawTrails();
        drawMotes();
    ...
    function addStone(p) {
        ...
        obstacles.push({
            ...
            points: Array.from({
                length: 7
            }, (_, i) => [i / 7 * TAU, r * (.78 + Math.random() * .32)])
        });
        lastPoint = p;
    }

    function bind() {
        const slider = (id, output, fn) => document.getElementById(id).addEventListener("input", e => {
            fn(+e.target.value);
            document.getElementById(output).textContent = id === "simulation-speed" ? settings.speed.toFixed(1) + "x" : id === "evaporation-rate" ? settings.evaporation.toFixed(2) : e.target.value;
        });
        slider("ant-count", "ant-count-value", v => {
            settings.antCount = v;
            resizeAnts();
        });
        ...
        document.getElementById("clear-pheromones").onclick = () => {
            home.fill(0);
            nectar.fill(0);
            seedHomeTrail();
        };
        document.getElementById("clear-obstacles").onclick = () => obstacles.length = 0;
        document.getElementById("reset-simulation").onclick = () => {
            obstacles.length = 0;
            delivered = 0;
            food.forEach(f => f.amount = f.max);
            home.fill(0);
            nectar.fill(0);
            seedHomeTrail();
            ants.forEach(a => a.reset());
        };
        ["mousedown", "touchstart"].forEach(type => canvas.addEventListener(type, e => {
            if (type[0] === "t") e.preventDefault();
            drawing = true;
            lastPoint = null;
            addStone(point(e));
        }, {
        ...
        ["mousemove", "touchmove"].forEach(type => canvas.addEventListener(type, e => {
            if (!drawing) return;
            if (type[0] === "t") e.preventDefault();
            addStone(point(e));
        }, {
        ...
        ["mouseup", "mouseleave", "touchend"].forEach(type => canvas.addEventListener(type, () => {
            drawing = false;
            lastPoint = null;
        }));
    }
