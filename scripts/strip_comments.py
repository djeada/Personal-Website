    ...
    for line in lines:
        ...
        while i < len(stripped_line):
            ...
            if in_multiline_string:
                cleaned_line += char
                if (
                    i + 2 <= len(stripped_line)
                    and stripped_line[i : i + 3] == in_multiline_string
                ):
...
def main():
    ...
    max_workers = min(
        32, len(all_files) or 1, cpu_count * 2
    )  # Respect system capabilities
